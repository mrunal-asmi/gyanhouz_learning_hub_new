      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <p className="text-xl text-muted-foreground">
            Industry-standard equipment that schools can't afford. Every student gets personal
            access.
          </p>
        </div>
            ...
            <div className="space-y-6">
              ...
              <div className="grid gap-4">
                {activeCategory?.equipment?.map((item, index) => (
                  ...
                  >
                    <div
                      className={`w-8 h-8 rounded-full bg-${activeCategory?.color}/10 flex items-center justify-center flex-shrink-0`}
                    >
                      <Icon
                        name="CheckCircleIcon"
                        size={20}
                        className={`text-${activeCategory?.color}`}
                      />
                    </div>
                    <div>
              ...
              {/* Value Proposition */}
              <div
                className={`bg-${activeCategory?.color}/10 border-l-4 border-${activeCategory?.color} p-6 rounded-r-lg`}
              >
                <div className="flex items-start space-x-3">
                  <Icon
                    name="SparklesIcon"
                    size={24}
                    className={`text-${activeCategory?.color} flex-shrink-0`}
                  />
                  <div>
                    <h4 className="font-bold text-foreground mb-2">Personal Equipment Access</h4>
                    <p className="text-sm text-muted-foreground">
                      Unlike schools where 30 students share 1 kit, each GYANHOUZ student gets
                      dedicated equipment for uninterrupted learning.
                    </p>
                  </div>
                </div>
              </div>
